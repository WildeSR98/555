...
- Кулдаун между действиями определяется таймером из маршрутного листа (см. ниже)
...
Каждый этап маршрута имеет **таймер** (в секундах) — минимальное время, которое устройство должно находиться на посту перед завершением. Таймер работает как на клиенте (UI-блокировка), так и на сервере (cooldown).

#### Архитектура

```
┌────────────────────┐       ┌─────────────────────────┐
│   Клиент (scan.html)│       │    Сервер (scan_api.py)  │
│                    │       │                         │
│  startStageTimer() │       │  ProjectRouteStage      │
│  ┌──────────────┐  │  WS   │  ├─ timer_seconds       │
│  │ Круговой     │◄─┼───────┼──┤ (из маршрута проекта) │
│  │ индикатор    │  │       │  └─ fallback: RouteConfig│
│  │ + блокировка │  │       │                         │
│  │ кнопок       │  │       │  can_change_status()    │
│  └──────────────┘  │       │  ├─ cooldown_seconds    │
│                    │       │  │  (из ProjectRouteStage│
│  timerStartedAt    │       │  │   или RouteConfig)    │
│  (в sessionStorage)│       │  └─ last_log.created_at  │
└────────────────────┘       └─────────────────────────┘
```

#### Откуда берётся значение таймера

Приоритет (от высшего к низшему):

1. **`ProjectRouteStage`** — индивидуальный маршрут проекта для данного типа устройства
2. **`RouteConfigStage`** — глобальный маршрут для данного типа устройства
3. **`RouteConfig` (is_default=True)** — дефолтный маршрут
4. **300 секунд** — жёсткий fallback, если ничего не найдено

#### Жизненный цикл таймера

```
1. Работник сканирует SN → POST /api/scan/action (scan_in)
   └─ Сервер возвращает stage_timer_seconds из ProjectRouteStage

2. Клиент запускает startStageTimer(seconds)
   ├─ Показывает круговой индикатор обратного отсчёта
   ├─ Блокирует кнопки Готово / Брак / Полуфабрикат
   ├─ Сохраняет timerStartedAt + stageTimerSeconds в sessionStorage
   └─ По истечении → разблокирует кнопки, pulse-анимация на «Готово»

3. При обновлении страницы (F5)
   ├─ restoreState() восстанавливает timerStartedAt из sessionStorage
   ├─ _restoreTimerFromServer() запрашивает GET /api/scan/device-timer
   │   └─ Получает актуальный timer_seconds из проектного маршрута
   └─ Вычисляет remaining = timer - elapsed, продолжает с оставшегося

4. При изменении маршрута (админ меняет таймер)
   ├─ PUT /api/project-routes/{id}/device/{type} → WS broadcast
   ├─ Клиент получает ws:project_route_saved с новыми stages
   ├─ Пересчитывает remaining = newTimer - elapsed
   ├─ Если remaining > 0 → продолжает таймер с новым значением
   └─ Если remaining ≤ 0 → сразу разблокирует кнопки

5. Работник нажимает «Готово» → POST /api/scan/action (complete)
   └─ Сервер проверяет cooldown_seconds из ProjectRouteStage
      ├─ Прошло достаточно → OK, переводит устройство
      └─ Не прошло → {ok: false, error: "Ожидайте таймер. Осталось X:XX"}
```

#### API-эндпоинт таймера

```
GET /api/scan/device-timer?device_id={id}&stage_key={key}

Ответ: {timer_seconds: 60, device_type: "SERVAL", project_id: 23}
```

Используется при восстановлении сессии после обновления страницы.
